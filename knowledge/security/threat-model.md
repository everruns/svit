...
| `TM-DOS-008` | A model-authored port request or standard-library evaluation monopolizes CPU, memory, or output | Port JSON input caps; persistent and model-visible output validation; linear-time bounded regex search; jq filter/result caps with recursive and generator constructs rejected | PARTIAL: an activation-local HTTP response may exceed the durable value envelope so it can be reduced before commit; its in-memory cost is `L-048`, while extension CPU work and jaq still require native-process outer containment |
...
| `TM-DOS-012` | Guest or host writes accumulate an unbounded committed root one bounded value at a time | Aggregate node and text-byte limits measured over the whole root at the same boundary that validates a commit, restore, or fork | MITIGATED for the committed process tree; mount content is external and bounded separately |
| `TM-DOS-009` | A hostile event, replay tail, fork chain, or query exhausts storage-process resources | Event byte cap, replay/fork/query count limits, bounded query text, and reference-checked envelope reclamation at every cut | PARTIAL: focused event-query and cut-reclamation evidence passes; aggregate database size, snapshot bytes, and wall-time budgets remain |
...
| `TM-ESC-003` | A model-authored port request reaches the host filesystem, executable search path, or inherited environment | Ports accept only committed process values or explicit JSON and expose no shell, filesystem, process, or environment interface; Svit Lisp data functions have no host authority | PARTIAL: focused tests pass; the surrounding agent runtime remains native in-process code |
...
| `TM-ESC-005` | An untrusted dispatch name reaches a function outside the caller's intended handler set or converts a hard failure into ordinary data | Ephemeral tables accept only explicit unique name/function pairs; unknown names fail closed; safe dispatch propagates restriction, execution, resource, and port-suspension failures | MITIGATED for Svit Lisp dispatch tables |
...
| `TM-EFF-005` | Retried or failed HTTP and nested model tools duplicate non-transactional external effects | Explicit host grants, bounded calls, result replay within one activation, documented transaction boundary, and host idempotency/reconciliation policy | PARTIAL: persisted model-authored script integration proves one host-authorized HTTP call across guest suspension/replay, including rollback of guest state after a later failure; durable effect receipts across persistence conflict or process recovery are not implemented |
...
| `TM-MSG-002` | Failed or concurrent inbox acknowledgement drops or reorders input | Host-only enqueue plus exact-head acknowledgement after a successful turn, with complete-root validation before commit | MITIGATED for the in-memory process API and local Turso Svit owner |
...
| `TM-CAP-002` | A model uses generic mutation or an unintended script beyond its domain workflow | Svit does not attenuate the reasoning loop with model-specific operation or script allowlists: it receives the complete generic process surface. This does not trust guest source, inputs, values, or interpreter safety; those remain untrusted at every language boundary. Host authority enters only through explicitly attached ports and mounts. | OUT OF SCOPE |
...
| `TM-CAP-004` | A model-supplied URL widens the network authority selected by the host | Individually registered allowlisted or explicitly unrestricted HTTP policy, URL validation, redirect refusal in the reusable transport, persistent/model-visible value validation, and policy integration tests | PARTIAL: compile-fail evidence keeps the standard bundle unavailable; unrestricted and attenuated policy plus reqwest redirect evidence pass; the research transport intentionally permits unbounded activation-local response bodies (`L-048`), so unrestricted research hosts accept SSRF and memory-pressure exposure by design, while custom transport DNS and redirect enforcement remains a host responsibility |
| `TM-CAP-005` | A forged or stale `/ports` catalog is mistaken for current host authority | Host-only projection from attached ports, refresh on every build/resume, descriptive-only semantics, and exact-catalog tests | MITIGATED for volatile and local Turso Svit owners |
...
| `TM-CAP-006` | A host port extension receives process mutation or ambient capability merely by being dispatched | Explicit host-only registration and a `PortContext` limited to committed reads and discovery; extra authority must be deliberately captured by trusted host code | MITIGATED for the Svit-supplied context; extension implementations remain trusted native code |
...
| `TM-AUD-001` | Guest code or model tools rewrite durable thread history or diverge its presentation from canonical events | Bounded `/thread` metadata, process-partitioned paged `EventLog`, immutable event blobs, session/ID/sequence validation, and durable-fork prefix tests | MITIGATED for volatile and local Turso Svit owners |
| `TM-AUD-002` | A retention cut silently removes canonical history the loop still needs, or one process reclaims history another still reads | Cuts are explicit host policy at a positive boundary inside the committed range, refused unless a compaction checkpoint already replaced the prefix and no fork inherits it; the boundary is durable so appended sequences never reuse a reclaimed one | MITIGATED for volatile and local Turso Svit owners |
| `TM-PERS-001` | Corrupted, reordered, deleted, or spliced persisted events reconstruct attacker-selected state | Content-addressed blobs, base-bound hash chain, stable positions, atomically published and fully validated recovery checkpoints, typed reduction of every tail record, and complete root/resulting-root validation at bounded recovery boundaries | PARTIAL: event and checkpoint corruption plus resume/reducer evidence passes; authenticated storage and systematic fault injection remain |
