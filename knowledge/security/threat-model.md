...
| `TM-CAP-002` | An untrusted model uses generic mutation or an unintended script beyond its domain workflow | Not part of Svit's threat model: guest code and the reasoning loop are trusted. Host authority is introduced only by explicitly attached ports and mounts. | OUT OF SCOPE |
