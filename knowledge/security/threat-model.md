...
| `TM-CAP-005` | A forged or stale `/bin` catalog is mistaken for current host authority | Host-only projection from attached built-ins, refresh on every build/resume, descriptive-only semantics, and exact-catalog tests | MITIGATED for volatile and local Turso Svit owners |
