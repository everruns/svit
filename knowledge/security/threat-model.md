...
| `TM-CAP-004` | A model-supplied URL or persistent string forges network authority | Default-deny host `HttpAllowlist`, URL validation, redirect refusal in the standard transport, response caps, and denied/allowed integration tests | PARTIAL — exact policy plus standard reqwest redirect and streamed-response evidence pass; custom transport DNS, redirect, and SSRF enforcement remains a host responsibility |
