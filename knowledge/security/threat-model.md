...
| `TM-DOS-008` | A model-authored native data-tool request monopolizes CPU, memory, or output | Linear-time bounded regex search; jq input, filter, result, and output caps; recursive and generator jq constructs rejected | PARTIAL — focused rejection evidence passes; jaq still requires native-process outer containment |
...
| `TM-ESC-003` | A model-authored native executable request reaches the host filesystem, executable search path, or inherited environment | Native executables accept only committed process values or explicit JSON and expose no shell, filesystem, process, or environment interface | PARTIAL — focused executable tests pass; the surrounding agent runtime remains native in-process code |
...
| `TM-CAP-004` | A model-supplied URL or persistent string forges network authority | Default-deny host `HttpAllowlist`, mandatory host transport, URL validation, response caps, and denied/allowed integration tests | PARTIAL — exact policy and transport evidence passes; transport-level DNS and SSRF enforcement remains a host responsibility |
| `TM-CAP-005` | A forged or stale `/bin` catalog is mistaken for current host authority | Host-only projection from attached executables, refresh on every build/resume, descriptive-only semantics, and exact-catalog tests | MITIGATED for the in-memory agent runtime |
