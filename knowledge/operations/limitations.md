...
| `L-033` | The Everruns loop records host-generated event identifiers and timestamps plus provider output | The paged EventLog preserves the recorded thread independently of process snapshots, but deterministic Svit Lisp replay does not imply deterministic model-turn replay |
| `L-034` | The live reasoning-turn outbox is a bounded in-memory notification stream | Slow listeners may lag; durable conversation recovery uses the paged EventLog and compaction checkpoint, not the live receiver or `/thread` metadata |
...
| `L-047` | Context compaction requires a provider-supported strategy | Turso durably stores Everruns checkpoints, but a provider without native compaction falls back to Everruns' configured policies; without a checkpoint no canonical history can be reclaimed, so a Svit whose provider never compacts stays unbounded |
...
| `L-051` | No aggregate bound on committed memory-tree size | `Limits` caps depth, entries, and bytes per value but not total node count or total tree bytes, so a script appending one small node per activation grows the committed root and every snapshot of it |
