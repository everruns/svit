...
| `L-005` | Svit Lisp reaches external systems only through host-attached mounts | Guest code resolves mount nodes under the descriptor's granted access and reads `/bin` manuals, but cannot execute `/bin` or access network, raw filesystem/database handles, models, secrets, or clocks; host-side built-in dispatch does not widen the Lisp surface |
...
| `L-046` | Process import preserves current state but not source history topology | Import starts a new event tail at the existing version; prior transaction envelopes, snapshots, and fork references remain only in the source store |
